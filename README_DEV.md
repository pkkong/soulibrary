# Soulib Development Guide

이 저장소의 기준 원본은 GitHub `pkkong/soulibrary`입니다. 기본 개발환경은 GitHub Codespaces입니다. 로컬 PC는 프로젝트 파일을 보관하는 장소가 아니라, GitHub/Codex/Codespaces에 접속하는 단말로 봅니다.
...
### Vercel + Supabase 이전 기록

Cloudtype에서 Vercel + Supabase로의 이전은 완료된 운영 이력입니다. 재실행용 one-off migration script는 공개 저장소에서 제거했습니다.

복구나 재구성이 필요하면 아래 순서로 새 작업을 잡습니다.

1. Supabase `shared_shelves` schema를 `supabase/migrations/` 기준으로 확인합니다.
2. Vercel project env를 `docs/production_operations.md` 기준으로 다시 설정합니다.
3. `main` push 후 GitHub Actions와 live smoke를 확인합니다.
