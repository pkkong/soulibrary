...
## 3) 데이터 흐름(핵심)
### A. 기본 검색
- 운영 기본값은 DB 없는 실시간 검색이다.
- 사용자가 검색하면 각 플랫폼/도서관 커넥터를 실시간 조회하고 결과를 짧게 사용한다.
- 이 흐름에는 Git에 체크인된 CSV가 필요 없다.

### B. 로컬 데이터 작업(예외)
- 과거 크롤링/적재 작업이 필요할 때만 로컬 `data/`에 CSV/JSON/DB를 생성한다.
- `data/*.csv`, `data/*.json`, `data/*.db`, 캐시/로그는 Git에 올리지 않는다.
- 대용량 데이터는 GitHub 코드 저장소가 아니라 별도 백업 위치나 실행 환경의 로컬 파일로 관리한다.
